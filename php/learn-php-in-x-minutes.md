...
// () 对于 echo 和 print 是可选的
echo "X minutes know php"; // 输出 “X minutes know php” 并且换行
// (每个语句必须以分号结尾)
...
echo $num++; //输出1 （运算后自增）
echo ++$num; // 输出3（自增后运算）
$num /= $float; // 先除后赋值给 $num

// 字符串需要被包含在单引号之中
$sql_quotes = '$String'; // => '$String'

// 如果需要在字符串中引用变量，就需要使用双引号
$dbl_quotes = "This is a $sql_quotes ."; // => 'This is a $String.'
...
// 自 PHP5.3 开始，nowdocs 可以被用作多行非计算型字符串
$nowdoc = <<<'END'
Multi line
string
END;
...
echo $my_class->prot; // => 这行会产生一个致命错误
echo $my_class->priv; // => 这行也会产生一个致命错误
...
如果你对最佳实践感兴趣（实时更新）[PHP The Right Way.](http://phptherightway.p2hp.com/)

如果你很熟悉善于包管理的语言 [Composer](https://docs.phpcomposer.com/)

如要了解通用标准，请访问PHP Framework Interoperability Group’s [PSR standards.](https://learnku.com/docs/psr/)

如果你想了解每个 PHP 版本的 [CHANGELOG](https://www.php.net/manual/zh/appendices.php)